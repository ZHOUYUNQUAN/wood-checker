...
(function () {
    ...
    var FIELD_KEYWORDS = [
        ...
        { field: 'diameter_avg',    kw: ['平均径', '综合径', '平均直径', '综合直径', '径级', 'diamètre', 'diameter', 'diametro', 'Diamètre', 'Diameter'] },
        { field: 'length_m',        kw: ['长度(m)', '长度(M)', '长度', 'length', '长(M)', '长(m)', 'long', 'largo', 'Long', 'Largo', 'réel'] },
    ...
    function detectColumn(header) {
        ...
        // 纯数字表头（如 "1","2","3","4"）→ 匹配 diameter_1~4
        var trimmed = header.trim();
        if (trimmed === '1') return 'diameter_1';
        if (trimmed === '2') return 'diameter_2';
        if (trimmed === '3') return 'diameter_3';
        if (trimmed === '4') return 'diameter_4';
    ...
    var FIELD_LABELS = {
        ...
        'customer':        { label: '供应商/基地', group: 'info' },
